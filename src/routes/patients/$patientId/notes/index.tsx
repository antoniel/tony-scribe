        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          <Card className="bg-slate-800/50 border-slate-700">
            <CardPanel className="p-12 text-center">
              <p className="text-slate-400 mb-6">Estudo não encontrado</p>
      ...
      <CardPanel className="p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-1">Data de Início: {formatDate(new Date(patient.dateOfBirth))}</p>
          </div>
...
function NotesListTable({ notes, patientId }: { notes: Note[]; patientId: string }) {
  if (notes.length === 0) {
    return (
      <Card className="bg-slate-800/50 border-slate-700">
        <CardPanel className="p-12 text-center">
          <p className="text-slate-400 mb-6">Nenhuma nota encontrada para este estudo. Crie a primeira nota para começar.</p>
  ...
  return (
    <Card className="bg-slate-800/50 border-slate-700">
      <CardHeader>
        <CardTitle>Notas de Estudo ({notes.length})</CardTitle>
      </CardHeader>
      <CardPanel>
              ...
              {notes.map((note) => (
                <tr key={note.id} className="border-b border-slate-700 hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    ...
                    >
                      {note.transcriptionStatus === 'pending' ? 'Pendente' : note.transcriptionStatus === 'completed' ? 'Completo' : 'Falhou'}
                    </Badge>
                  </td>
